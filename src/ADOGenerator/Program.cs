...
do
{
    ...
    Console.WriteLine("2. Generate a new Artifacts using exsisting project.");
    ...
    else if (userChoiceTemplate == "2")
    {
        ...
        var projectDetails = await projService.SelectProject(accessToken, projects);
        projectName = projectDetails[1];
        ...
        //var trackingId = Guid.NewGuid().ToString().Split('-')[0];
        Project model = new Project();
        model.accountName = organizationName;
        model.ProjectName = projectName;
        model.ProjectId = projectDetails[0];
        model.accessToken = accessToken;
        model.adoAuthScheme = authScheme;
        model.id = id;
        ITemplateService templateService = new TemplateService(configuration);
        var analyzed = templateService.AnalyzeProject(model);
        if (analyzed)
        {
            id.AddMessage("Artifacts generated successfully.");
            //templateService.StartEnvironmentSetupProcess(model);
