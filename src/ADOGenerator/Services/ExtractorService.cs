...
namespace ADOGenerator.Services
{
    public class ExtractorService : IExtractorService
    {
        ...
        {
            extractedTemplatePath = currentPath + @"ExtractedTemplate\";
            ...
            ExportTeams(appConfig.BoardConfig, model);
                    ...
                    new
                    {
                        ...
                        Template = new[]
                        {
                            new
                            {
                                ...
                                ShortName = model.ProjectName.Replace(" ", ""),
                                Description = $"This is a custom template for the project '{model.ProjectName}'.",
                                //Tags = new[] { "custom", "template", "generated" },
                                Author = "Generated by Azure DevOps Demo Generator",
                                //Image = "/Templates/TemplateImages/CustomTemplate.png",
                                //PreviewImages = new[] { "/Templates/CustomTemplate/Images/Preview1.png", "/Templates/CustomTemplate/Images/Preview2.png" },
                                Message = "This template was generated based on the artifacts of the selected project."
                            }
                        }
                    }
                }
            };
            Console.WriteLine("JSON block to be added to TemplateSettings:");
            Console.WriteLine(JsonConvert.SerializeObject(customTemplateJson, Formatting.Indented));
            ...
            try
            {
                ...
                if (returnExtensionsList != null && returnExtensionsList.count > 0)
                {
                    ...
                    if (extensionList.Count > 0)
                    {
                        ...
                        if (!Directory.Exists(extractedTemplatePath + $"CT-{appConfig.ExtensionConfig.Project.Replace(" ", "-")}"))
                        {
                            Directory.CreateDirectory(extractedTemplatePath + $"CT-{appConfig.ExtensionConfig.Project.Replace(" ", "-")}");
                        }
        ...
        public bool ExportTeams(Configuration con, Project model)
        {
            try
            {
